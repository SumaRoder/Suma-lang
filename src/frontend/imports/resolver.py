...
from src.frontend.parser.ast_nodes import ImportDecl, Program, TopLevel
...
class ImportResolver:
    ...
    def with_defaults(cls, import_paths: Iterable[str] | None = None) -> "ImportResolver":
        paths: list[str] = []
        env_paths = os.environ.get("SUMA_PATH", "")
        if env_paths:
...
def _default_import_paths() -> list[str]:
    ...
    return [str(Path.cwd()), str(project_root), str(project_root / "stdlib")]
