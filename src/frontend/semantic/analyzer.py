...
class Analyzer:
    ...
    def _base_type(self, type_name: Optional[str]) -> Optional[str]:
    ...
    def _split_type_args(self, type_name: Optional[str]) -> list[str]:
    ...
    def _result_ok_type(self, type_name: Optional[str]) -> Optional[str]:
        base = self._base_type(type_name)
        args = self._split_type_args(type_name)
    ...
    def _result_err_type(self, type_name: Optional[str]) -> Optional[str]:
        base = self._base_type(type_name)
        args = self._split_type_args(type_name)
    ...
    def _function_arg_types(self, type_name: Optional[str]) -> list[str]:
        if self._base_type(type_name) != "Function":
            return []
        args = self._split_type_args(type_name)
        return args[:-1] if args else []

    def _function_return_type(self, type_name: Optional[str]) -> Optional[str]:
        if self._base_type(type_name) != "Function":
            return None
        args = self._split_type_args(type_name)
    ...
    def _lambda_type(
        self, params: Sequence[Param], return_type: Optional[str], result_type: Optional[str]
    ) -> str:
    ...
    def _erase_type(self, type_name: Optional[str]) -> Optional[str]:
        if type_name is None:
            return None
        return self._base_type(type_name)

    def _substitute_type(
        self, type_name: Optional[str], mapping: dict[str, Optional[str]]
    ) -> Optional[str]:
        if type_name is None:
            return None
        ...
        if not args:
            return type_name
        base = self._base_type(type_name) or type_name
        substituted = [self._substitute_type(arg, mapping) or arg for arg in args]
        return f"{base}<{','.join(substituted)}>"

    def _class_type_mapping(self, obj_type: Optional[str]) -> dict[str, Optional[str]]:
        ...
        if cls is None or not cls.type_params:
            return {}
        args = self._split_type_args(obj_type)
        return {
            name: args[index] if index < len(args) else None
            for index, name in enumerate(cls.type_params)
        }

    def _base_class_name(self, class_name: str) -> Optional[str]:
        cls = self._class_decls.get(class_name)
        if cls is None or cls.base_type is None:
            return None
        return self._base_type(cls.base_type)

    def _class_chain(self, class_name: Optional[str]) -> list[str]:
        chain: list[str] = []
        seen: set[str] = set()
        current = class_name
    ...
    def _project_type_to_base(
        self, actual_type: Optional[str], target_base: Optional[str]
    ) -> Optional[str]:
        ...
        while current_type is not None:
            current_base = self._base_type(current_type)
            ...
            if cls is None or cls.base_type is None:
                return None
            current_type = self._substitute_type(
                cls.base_type, self._class_type_mapping(current_type)
            )
        return None

    def _class_type_mapping_for(
        self, obj_type: Optional[str], owner_class: str
    ...
    def _bind_method_to_object(
        self, obj_type: Optional[str], owner_class: str, method: FunctionDecl
    ) -> FunctionDecl:
        mapping = self._class_type_mapping_for(obj_type, owner_class)
        if not mapping:
            return method
        params = tuple(
            replace(param, type_annotation=self._substitute_type(param.type_annotation, mapping))
            for param in method.params
        )
        return replace(
            method,
            params=params,
            return_type=self._substitute_type(method.return_type, mapping),
        )

    def _method_signature(self, func: FunctionDecl) -> tuple[int, tuple[Optional[str], ...]]:
        return len(func.params), self._erased_signature(func)

    def _class_method_candidates(
        self, obj_type: Optional[str], method_name: str
    ) -> list[tuple[str, FunctionDecl]]:
        base_type = self._base_type(obj_type)
        candidates: list[tuple[str, FunctionDecl]] = []
        seen_signatures: set[tuple[int, tuple[Optional[str], ...]]] = set()
        for owner_class in self._class_chain(base_type):
    ...
    def _class_field_candidate(
        self, obj_type: Optional[str], member: str
    ...
    def _erased_signature(self, func: FunctionDecl) -> tuple[Optional[str], ...]:
        return tuple(self._erase_type(param.type_annotation) for param in func.params)

    def _validate_overload_set(self, owner: str, funcs: Sequence[FunctionDecl]) -> None:
        if len(funcs) <= 1:
            return
        signatures: dict[tuple[int, tuple[Optional[str], ...]], FunctionDecl] = {}
        for func in funcs:
            if func.decorators:
    ...
    def _validate_inheritance(self) -> None:
        for class_name, cls in self._class_decls.items():
            if cls.base_type is None:
                continue
            base_name = self._base_type(cls.base_type)
            ...
            if base_name == class_name:
                ...
                continue
            base_args = self._split_type_args(cls.base_type)
        ...
        for class_name, cls in self._class_decls.items():
            ...
            inherited_methods: dict[
                tuple[str, tuple[int, tuple[Optional[str], ...]]], FunctionDecl
            ] = {}
            for base_name in self._class_chain(self._base_class_name(class_name)):
    ...
    def _match_type_pattern(
        ...
        actual_base = self._base_type(actual)
        if expected_base != actual_base:
            return self._is_assignable(expected, actual)
        expected_args = self._split_type_args(expected)
        actual_args = self._split_type_args(actual)
        ...
        return all(
            ...
            for exp, act in zip(expected_args, actual_args)
        )

    def _overload_score(
        self,
        func: FunctionDecl,
        arg_types: Sequence[Optional[str]],
        initial_mapping: Optional[dict[str, Optional[str]]] = None,
    ) -> tuple[int, dict[str, Optional[str]]] | None:
        required = sum(
        ...
        for arg_type, param in zip(arg_types, func.params):
            expected = self._substitute_type(param.type_annotation, bindings)
            if not self._match_type_pattern(expected, arg_type, func.type_params, bindings):
                return None
            expected_after = self._substitute_type(param.type_annotation, bindings)
            if expected_after == arg_type:
                score += 4
            elif self._erase_type(expected_after) == self._erase_type(arg_type):
                score += 3
    ...
    def _resolve_overload(
        ...
        expr: CallExpr,
        initial_mapping: Optional[dict[str, Optional[str]]] = None,
    ) -> tuple[Optional[FunctionDecl], dict[str, Optional[str]], list[Optional[str]]]:
        arg_types = [self._infer_expr(arg) for arg in expr.args]
        matches: list[tuple[int, FunctionDecl, dict[str, Optional[str]]]] = []
        for func in funcs:
    ...
    def _infer_function_call(
        ...
        if len(funcs) > 1:
            ...
            if selected is None:
                return None
            return self._substitute_type(selected.return_type, bindings)
        bindings = self._check_function_call(funcs[0], expr, initial_mapping)
        return self._substitute_type(funcs[0].return_type, bindings)
    ...
    def _validate_class_accessors(self, cls: ClassDecl) -> None:
        ...
        setter_types: dict[str, Optional[str]] = {}
    ...
    def _infer_expr(self, expr: Expr) -> Optional[str]:
        ...
        if isinstance(expr, UnaryExpr):
            ...
            if candidates:
                ...
                return self._infer_function_call(
                    f"{self._base_type(operand_type)}.{method_name}",
        ...
        if isinstance(expr, LambdaExpr):
            ...
            return self._lambda_type(lambda_params, expr.return_type, result_type)
        if isinstance(expr, ElvExpr):
            left_type = self._infer_expr(expr.left)
            right_type = self._infer_expr(expr.right)
            if self._base_type(left_type) not in (None, "R", "Ok", "Err"):
                self._error("'?:' operator requires a Result value", expr.info)
            return self._common_type(self._result_ok_type(left_type), right_type)
        ...
        if isinstance(expr, PropagateExpr):
            value_type = self._infer_expr(expr.value)
            if self._base_type(value_type) not in (None, "R", "Ok", "Err"):
                self._error("'?' operator requires a Result value", expr.info)
            ...
            return self._result_ok_type(value_type)
        if isinstance(expr, SafeCallExpr):
            ...
            if candidates:
                ...
                return self._infer_function_call(
                    f"{self._base_type(receiver_type)}.{expr.member}",
                    [method for _, method in candidates],
                    fake_call,
                )
            for arg in expr.args:
                self._infer_expr(arg)
            if self._base_type(receiver_type) in (None, "R", "Ok", "Err", "PyObject", "PyCallable"):
                return None
        ...
        if isinstance(expr, SafeMemberExpr):
            obj_type = self._infer_expr(expr.obj)
            receiver_type = self._safe_receiver_type(obj_type)
            if self._base_type(receiver_type) in (
                None,
                "R",
                "Ok",
                "Err",
    ...
    def _match_it_type(self, scrutinee_type: Optional[str], pattern: MatchPattern) -> Optional[str]:
        if pattern.kind == "result":
            if pattern.value == "Ok":
                return self._result_ok_type(scrutinee_type)
            if pattern.value == "Err":
                return self._result_err_type(scrutinee_type)
        if pattern.kind == "type" and isinstance(pattern.value, str):
            return pattern.value
        return scrutinee_type

    def _safe_receiver_type(self, obj_type: Optional[str]) -> Optional[str]:
        base_type = self._base_type(obj_type)
        if base_type in ("R", "Ok"):
            return self._result_ok_type(obj_type)
        return obj_type

    def _infer_if_expr(self, expr: IfExpr) -> Optional[str]:
    ...
    def _iter_item_type(self, iterable_type: Optional[str]) -> Optional[str]:
        base = self._base_type(iterable_type)
        if base == "Range":
            return "Int"
        if base == "Str":
            return "Str"
        if base == "List":
            args = self._split_type_args(iterable_type)
    ...
    def _infer_binary(self, expr: BinaryExpr) -> Optional[str]:
        ...
        if candidates:
            ...
            return self._infer_function_call(
                f"{self._base_type(left_type)}.{method_name}",
                [method for _, method in candidates],
                fake_call,
            )
        if expr.op in ("+", "-", "*", "/", "%", "&", "|", "^", "<<", ">>"):
    ...
    def _infer_call(self, expr: CallExpr) -> Optional[str]:
        if isinstance(expr.callee, MemberExpr):
            ...
            if candidates:
                ...
                return self._infer_function_call(
                    f"{self._base_type(obj_type)}.{expr.callee.member}",
                    [method for _, method in candidates],
                    expr,
                )
        ...
        if isinstance(expr.callee, Identifier):
            ...
            if name in self.BUILTINS:
                ...
                if name == "unwrap":
                    if arg_types and self._base_type(arg_types[0]) not in (None, "R", "Ok", "Err"):
                        self._error("'unwrap' expects a Result value", expr.args[0].info)
                    return self._result_ok_type(arg_types[0] if arg_types else None)
                if name == "unwrap_or":
                    result_type = arg_types[0] if arg_types else None
                    default_type = arg_types[1] if len(arg_types) > 1 else None
                    if self._base_type(result_type) not in (None, "R", "Ok", "Err"):
                        self._error("'unwrap_or' expects a Result value", expr.args[0].info)
                    ok_type = self._result_ok_type(result_type)
        ...
        arg_types = [self._infer_expr(arg) for arg in expr.args]
        if self._base_type(callee_type) == "Function":
    ...
    def _infer_function_value_call(
        self, callee_type: Optional[str], arg_types: Sequence[Optional[str]], expr: CallExpr
    ) -> Optional[str]:
        expected_arg_types = self._function_arg_types(callee_type)
        return_type = self._function_return_type(callee_type)
        ...
        if len(arg_types) != len(expected_arg_types):
            ...
            return return_type
        for arg, expected, actual in zip(expr.args, expected_arg_types, arg_types):
            self._check_assignable(
                expected,
                actual,
                arg.info,
    ...
    def _infer_constructor_call(self, class_name: str, expr: CallExpr) -> Optional[str]:
        cls = self._class_decls.get(class_name)
    ...
    def _member_type(self, obj_type: Optional[str], member: str, info: TokenInfo) -> Optional[str]:
        if obj_type is None:
            return None
        base_type = self._base_type(obj_type)
        if base_type in ("PyModule", "PyObject", "PyCallable"):
            return "PyObject"
        if base_type == "Str":
            if member == "size":
                return "Int"
        ...
        if method_candidates:
            ...
            return "Function"
        if base_type in self._class_decls:
            self._error(f"No member '{member}' on {obj_type}", info)
        return None

    def _assignment_member_type(
        self, obj_type: Optional[str], member: str, info: TokenInfo
    ) -> Optional[str]:
    ...
    def _check_member_access(
        self, class_name: str, is_pub: bool, member: str, obj_type: Optional[str], info: TokenInfo
    ) -> None:
    ...
    def _is_assignable(self, expected: Optional[str], actual: Optional[str]) -> bool:
        ...
        if expected_base == "Function" and actual_base == "Function":
            ...
            if len(expected_args) != len(actual_args):
                return False
            return all(
                self._is_assignable(exp, act) for exp, act in zip(expected_args, actual_args)
            )
        if expected_base == "R":
            expected_args = self._split_type_args(expected)
            if actual_base == "R":
            ...
            if actual_base == "Ok":
                ok_type = self._result_ok_type(actual)
                if not expected_args:
                    return True
                ...
                if len(expected_args) >= 2:
                    return self._is_assignable(expected_args[0], ok_type)
            if actual_base == "Err":
                err_type = self._result_err_type(actual)
                if not expected_args:
                    return True
                if len(expected_args) == 1:
        ...
        if projected_actual is not None:
            expected_args = self._split_type_args(expected)
            actual_args = self._split_type_args(projected_actual)
            if not expected_args or not actual_args:
                return True
            return expected_args == actual_args
        return False
